...
- **Генерация Go из proto**: `make proto` (локальный `protoc` или Docker из `infra-external`). В целях proto добавлен `-I third_party` для `google/api/annotations.proto`.
...
- **OpenAPI из proto**: `make proto-openapi` (нужны `protoc` и `protoc-gen-openapiv2`). Результат: `api/openapi.json` / `api/openapi.swagger.json`. Swagger UI: `http://localhost:8080/swagger`.
